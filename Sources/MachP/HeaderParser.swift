...
public struct HeaderParser {
    ...
    public static func parseMachOHeader(from fileData: Data, at offset: Int, debugEnabled: Bool) throws -> [String: Any] {
        ...
        dbg("Header data available from \(offset) to \(offset + headerSize)")
        ...
        dbg(String(format: "Header magic: 0x%08x, bigEndian: \(isBigEndian)", headerMagic))
        ...
        dbg("Reading header fields with \(isBigEndian ? "big-endian" : "little-endian") ordering")
        ...
        dbg("Parsed header: magic=0x\(String(format: "%08x", magic)), cputype=\(cputype), cpusubtype=\(cpusubtype), filetype=\(filetype), ncmds=\(ncmds), sizeofcmds=\(sizeofcmds), flags=0x\(String(format: "%08x", flags)), reserved=\(reserved)")
